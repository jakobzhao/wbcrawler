            ...
            if not SGD:
                if fine_tune and layers != []:
                    ...
                    for i, layer in enumerate(layers[::-1]):
                        if len(layers) - i - 2 >= 0:
                            ...
                            activations = X
                        layer.weights_, layer_alphas[i], layer_betas[i], back_error = layer.backPropagate(layer.weights_, next_weights, activations, Y_new, rho, layer_alphas[i], layer_betas[i], dropout, back_delta=back_error,
                                                                                                          fine_tune=True)
                ...
                if fine_tune and layers != []:
                    ...
                    for i, layer in enumerate(layers[::-1]):
                        ...
                        layer.weights_, layer_alphas[i], layer_betas[i], back_error = layer.backPropagate(layer.weights_, next_weights, activations, Y_new[samples, :], rho, layer_alphas[i], layer_betas[i], dropout, back_delta=back_error,
                                                                                                          fine_tune=True)
                        next_weights = layer.weights_[0]
                elif not fine_tune:
                    if self.auto_encoder:
